...
namespace WriteBalance.Infrastructure.Services
{
    public class BalanceGenerator : IBalanceGenerator
    {
        ...
        public BalanceGenerator(BalanceMerge balanceMerge, BalanceCheck balanceCheck, CalculateNewRows calculateNewRows)
        {
        ...
        public async Task<MemoryStream> GenerateGardeshTablesAsync(List<FinancialRecord> financialRecords, IExcelExporter excelExporter, DBRequestDto requestDB)
        {
            try
            {
                Logger.WriteEntry(JsonConvert.SerializeObject("Starting GenerateTablesAsync"), $"BalanceGenerator:GenerateTablesAsync --typeReport:Info");
