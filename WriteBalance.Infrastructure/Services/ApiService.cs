...
namespace WriteBalance.Infrastructure.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<bool> GetVerifyUniqueNameAsync(string token, string  FolderPath, string BalanceName)
        {
            try
            {
                ...
                if (result == false )
                {
                    Logger.WriteEntry(JsonConvert.SerializeObject($"Failed to fetch verify for unique name from (name={BalanceName})"), $"ApiService:GetVerifyUniqueNameAsync --typeReport:Error");
        ...
        public async Task<bool> PostFileAsync(string token, string file, string FileName, string BalanceName, string description, int currencyType, string FolderPath)
        {
            try
            {
                Logger.WriteEntry(JsonConvert.SerializeObject($"Starting PostFileAsync fileName: {FileName}"), $"ApiService: PostFileAsync--typeReport:Info");
